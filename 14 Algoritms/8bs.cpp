...
std::vector<Node*> beamSearch(Node* start, Node* goal, int beam_width) {
...
int main() {
    ...
    std::cout << "Running Beam Search (w=2) from S to G..." << std::endl;
    std::vector<Node*> path = beamSearch(&s_node, &g_node, 2);
    printPath("Beam Search", path);
